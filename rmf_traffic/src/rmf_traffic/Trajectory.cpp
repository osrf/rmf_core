...
class Trajectory::Segment::Implementation {
 public:
  ...
  internal::SegmentElement::Data& data() { return myself->data; }

  const internal::SegmentElement::Data& data() const { return myself->data; }

  Time time() const { return data().finish_time; }
};
...
class Trajectory::Implementation {
 public:
  ...
  Implementation(std::string map_name) : map_name(std::move(map_name)) {
    // Do nothing
  }

  Implementation(const Implementation& other) { *this = other; }
  ...
  iterator find(Time time) {
    ...
    if (it == ordering.end()) return make_iterator<Segment>(segments.end());
  ...
  iterator erase(iterator first, iterator last) {
    ...
    const auto order_end = seg_end == segments.end()
                               ? ordering.end()
                               : ordering.find(seg_end->data.finish_time);

    ordering.erase(order_start, order_end);
    return make_iterator<Segment>(segments.erase(seg_begin, seg_end));
  }

  iterator begin() { return make_iterator<Segment>(segments.begin()); }

  iterator end() { return make_iterator<Segment>(segments.end()); }
};
...
Trajectory::ProfilePtr Trajectory::Profile::make_queued(
    geometry::ConstConvexShapePtr shape, const std::string& queue_id) {
  ProfilePtr result(new Profile(std::move(shape)));
...
//==============================================================================
Trajectory::Profile::StrictInfo::StrictInfo(void* pimpl) : _pimpl(pimpl) {
  // Do nothing
}
...
//==============================================================================
Trajectory::Profile::QueueInfo::QueueInfo(void* pimpl) : _pimpl(pimpl) {
  // Do nothing
}

//==============================================================================
auto Trajectory::Profile::get_queue_info() const -> const QueueInfo* {
  if (Agency::Queued == _pimpl->agency_mode) return &_pimpl->queue_info;
...
//==============================================================================
Time Trajectory::Segment::get_finish_time() const { return _pimpl->time(); }
...
void Trajectory::Segment::adjust_finish_times(Duration delta_t) {
  ...
  if (delta_t.count() < 0 && begin_it != segments.begin()) {
    ...
    if (new_time <= predecessor_it->data.finish_time) {
      const auto tp =
          predecessor_it->data.finish_time.time_since_epoch().count();
...
std::unique_ptr<Motion> Trajectory::Segment::compute_motion() const {
  ...
  if (it == segments.begin()) {
    return std::make_unique<SinglePointMotion>(
        finish_data.finish_time, finish_data.position, finish_data.velocity);
  }
...
//==============================================================================
std::string Trajectory::get_map_name() const { return _pimpl->map_name; }
...
Trajectory::InsertionResult Trajectory::insert(Time finish_time,
  ...
  return _pimpl->insert(
      internal::SegmentElement::Data{std::move(finish_time), std::move(profile),
                                     std::move(position), std::move(velocity)});
}

//==============================================================================
Trajectory::iterator Trajectory::find(Time time) { return _pimpl->find(time); }
...
//==============================================================================
Trajectory::iterator Trajectory::begin() { return _pimpl->begin(); }
...
//==============================================================================
Trajectory::iterator Trajectory::end() { return _pimpl->end(); }
...
Duration Trajectory::duration() const {
  const auto& segments = _pimpl->segments;
  return segments.size() < 2 ? Duration(0)
                             : segments.back().data.finish_time -
                                   segments.front().data.finish_time;
}

//==============================================================================
std::size_t Trajectory::size() const { return _pimpl->segments.size(); }
