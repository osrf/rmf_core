...
struct State {
  ...
  State(double s_in, double v_in, double t_in, Time t_start)
      : s(s_in),
        v(v_in),
        t(t_start + std::chrono::duration_cast<std::chrono::nanoseconds>(
...
//==============================================================================
States compute_traversal(const Time start_time, const double s_f,
                         const double v_nom, const double a_nom) {
  States states;
  states.reserve(3);
...
//==============================================================================
void interpolate_translation(Trajectory& trajectory, const double v_nom,
                             const double a_nom, const Time start_time,
...
//==============================================================================
void interpolate_rotation(Trajectory& trajectory, const double w_nom,
                          const double alpha_nom, const Time start_time,
                          const Eigen::Vector3d& start,
                          const Eigen::Vector3d& finish) {
...
class invalid_traits_error::Implementation {
 public:
  static invalid_traits_error make_error(const VehicleTraits& traits) {
    ...
    for (const auto& pair :
      ...
      if (pair.second <= 0.0) values.push_back(pair);
    }
...
class Interpolate::Options::Implementation {
 public:
  Implementation(const bool always_stop, const double translation_thresh,
                 const double rotation_thresh, const double corner_angle_thresh)
...
//==============================================================================
Interpolate::Options::Options(const bool always_stop,
                              const double translation_thresh,
                              const double rotation_thresh,
                              const double corner_angle_thresh)
    : _pimpl(rmf_utils::make_impl<Implementation>(
          always_stop, translation_thresh, rotation_thresh,
          corner_angle_thresh)) {
  // Do nothing
}
...
//==============================================================================
bool Interpolate::Options::always_stop() const { return _pimpl->_always_stop; }
...
//==============================================================================
Interpolate::Options& Interpolate::Options::set_rotation_threshold(
    double angle) {
...
Trajectory Interpolate::positions(
  ...
  if (input_positions.empty()) return trajectory;

  trajectory.insert(start_time, nullptr, input_positions.front(),
                    Eigen::Vector3d::Zero());
  ...
  for (std::size_t i = 1; i < N; ++i) {
    ...
    if (!always_stop && i + 1 < N) {
      ...
      bool can_skip = (next_p - last_p).norm() < translation_thresh ||
      ...
      if (d_next_p_norm > 1e-8 && d_future_p_norm > 1e-8) {
        ...
        if (angle < min_corner) can_skip = true;
      }
      ...
      if (can_skip && (std::abs(next_angle - last_angle) > rotation_thresh ||
    ...
    interpolate_translation(trajectory, v, a, *trajectory.finish_time(),
                            last_position, next_position);

    interpolate_rotation(trajectory, w, alpha, *trajectory.finish_time(),
                         last_position, next_position);
