...
std::string generate_self_intersection_polygon_message(
  ...
  std::string output =
      "[rmf_traffic::Polygon] Invalid polygon "
      "requested: " +
  ...
  for (const SimplePolygon::IntersectingPair& pair : intersections) {
    for (std::size_t i = 0; i < 2; ++i) {
      ...
      output += "(" + std::to_string(pair[i].indices[0]) + ") <" +
...
//==============================================================================
bool compute_intersection(const SimplePolygon::EdgeInfo& edge_a,
  ...
  if (std::abs(M.determinant()) < 1e-8) return false;
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (t_i < 0.0 || 1.0 < t_i) return false;
  }

  if (intersection_point) *intersection_point = p_a0 + t[0] * (p_a1 - p_a0);
...
//==============================================================================
bool is_polygon_ear(const Subpolygon& polygon, const Triangle& triangle) {
  ...
  for (std::size_t b0 = 0; b0 < polygon.size(); ++b0) {
    ...
    if (compute_intersection(edge_a, edge_b)) return false;
  }
...
//==============================================================================
std::size_t find_deepest_reflex_point(const Subpolygon& polygon,
  ...
  for (std::size_t i = 0; i < polygon.size(); ++i) {
    ...
    if (i == triangle[0] || i == triangle[1] || i == triangle[2]) continue;
    ...
    if (s[0] + s[1] > 1.0 || s[0] < 0.0 || s[1] < 0.0) continue;
...
std::array<Subpolygon, 2> split_subpolygon(
    const Subpolygon& original, const std::array<std::size_t, 2>& edge) {
...
//==============================================================================
Triangle get_original_indices(const Subpolygon& polygon,
                              const Triangle& triangle) {
  return Triangle{polygon[triangle[0]].original_index,
...
std::vector<Triangle> decompose_polygon(
  ...
  while (!subpolygon_queue.empty()) {
    ...
    const Triangle next_triangle = {pivot_point - 1, pivot_point,
                                    pivot_point + 1};
    ...
    const auto new_subpolygons =
        split_subpolygon(next_polygon, {pivot_point, reflex_point});
...
bool is_polygon_convex(const std::vector<Eigen::Vector2d>& polygon) {
  ...
  for (std::size_t i = 2; i < polygon.size(); ++i) {
    ...
    if (is_ccw != must_be_ccw) return false;
...
class ConvexWrapper : public fcl::Convex {
 public:
  ...
  static std::shared_ptr<fcl::Convex> make(
    ...
    return std::make_shared<ConvexWrapper>(std::move(fcl_points),
                                           2 * points.size());
  }
...
std::vector<std::shared_ptr<fcl::Convex>> make_triangulation(
  ...
  for (const Triangle& triangle : triangles) {
    points.clear();
    for (const std::size_t index : triangle) points.push_back(polygon[index]);
...
InvalidSimplePolygonException::InvalidSimplePolygonException(
    SimplePolygon::Intersections intersections, const std::size_t num_vertices)
...
class SimplePolygonInternal : public Shape::Internal {
 public:
  ...
  SimplePolygonInternal(std::vector<Eigen::Vector2d> points) : _points(points) {
    // Do nothing
  }

  bool check_self_intersections(Intersections* intersections) const {
    if (intersections) intersections->clear();
    ...
    if (intersections) return !intersections->empty();
  ...
  void except_on_invalid_polygon() const {
    if (_points.size() < 3) throw InvalidSimplePolygonException(_points.size());
  ...
  CollisionGeometries make_fcl() const final {
    ...
    if (is_polygon_convex(_points)) {
      ...
      for (auto&& triangle : triangles) shapes.push_back(std::move(triangle));
    }
...
Eigen::Vector2d& SimplePolygon::get_point(const std::size_t index) {
  return static_cast<SimplePolygonInternal*>(_get_internal())
      ->_points.at(index);
}
...
//==============================================================================
void SimplePolygon::insert_point(const std::size_t index,
