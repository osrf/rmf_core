...
//==============================================================================
SCENARIO("Differential Drive Heuristic")
{
  ...
  using DifferentialDriveCache =
    rmf_traffic::agv::planning::CacheManagerMap<
      rmf_traffic::agv::planning::DifferentialDriveHeuristicFactory>;
  DifferentialDriveCache diff_drive_cache(
    std::make_shared<
      rmf_traffic::agv::planning::DifferentialDriveHeuristicFactory>(
  ...
  {
    std::cout << " ==================== " << std::endl;
    auto solution = diff_drive_cache.get(goal_index)->get().get(key);
    std::cout << " --------------------\n"
