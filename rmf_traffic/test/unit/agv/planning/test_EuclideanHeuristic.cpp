...
SCENARIO("Euclidean Heuristic -- Single Floor")
{
  ...
  CHECK(value.value() == Approx(0.0).margin(1e-8));

  value = cache->get(1);
  REQUIRE(value.has_value());
  CHECK(value.value() == Approx((p1 - p0).norm()/max_speed));
  ...
  value = cache->get(2);
  REQUIRE(value.has_value());
  CHECK(value.value() == Approx((p2 - p0).norm()/max_speed));

  value = cache->get(3);
  REQUIRE(value.has_value());
  CHECK(value.value() == Approx((p3 - p0).norm()/max_speed));
}

//==============================================================================
SCENARIO("EuclideanHeuristic -- Easy Multifloor")
{
  rmf_traffic::agv::Graph graph;
...
//==============================================================================
SCENARIO("EuclideanHeuristic -- Complex Multifloor")
{
...
//==============================================================================
SCENARIO("EuclideanHeuristic -- No Connection")
{
