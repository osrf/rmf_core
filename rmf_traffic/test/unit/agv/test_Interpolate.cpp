...
struct PredictedTimes {
  /// time (relative to the start of the trajectory) that the vehicle started at
  /// rest
  double t_start;
  ...
  /// time (relative to the start of the trajectory) that the vehicle came to
  /// rest
  double t_finish;
};

void test_interpolation(const Eigen::Vector3d& p0, const Eigen::Vector3d& dir,
  ...
  for (std::size_t i = 1; i < NumTests; ++i) {
    ...
    const auto t =
        rmf_traffic::time::apply_offset(trajectory_start_time, t_rel + t_start);
    ...
    if (t_rel <= t_a) {
      ...
      const double s = 0.5 * a_n * pow(t_a, 2) + v_peak * (t_rel - t_a) -
                       0.5 * a_n * pow(t_rel - t_d, 2);
...
PredictedTimes compute_predicted_times(const double t_start,
...
SCENARIO("Test Interpolations") {
  ...
  GIVEN("Three distant points") {
    ...
    const std::vector<Eigen::Vector3d> positions = {
        Eigen::Vector3d{0.0, 0.0, 0.0}, Eigen::Vector3d{xf, 0.0, angle1},
        Eigen::Vector3d{xf, -xf, angle2}};

    rmf_traffic::Trajectory trajectory =
        rmf_traffic::agv::Interpolate::positions("test_map", traits, start_time,
                                                 positions);
    ...
    THEN("The trajectory is correctly interpolated") {
      ...
      test_interpolation(positions[0], Eigen::Vector3d::UnitX(), a_n, times1,
                         trajectory);

      const PredictedTimes times2 =
          compute_predicted_times(times1.t_finish, angle1, w_n, alpha_n);
      test_interpolation(Eigen::Vector3d{xf, 0.0, 0.0},
                         Eigen::Vector3d::UnitZ(), alpha_n, times2, trajectory);

      const PredictedTimes times3 =
          compute_predicted_times(times2.t_finish, xf, v_n, a_n);
      test_interpolation(positions[1], -Eigen::Vector3d::UnitY(), a_n, times3,
                         trajectory);

      const PredictedTimes times4 = compute_predicted_times(
          times3.t_finish, std::abs(angle2 - angle1), w_n, alpha_n);
      test_interpolation(Eigen::Vector3d{xf, -xf, angle1},
                         -Eigen::Vector3d::UnitZ(), alpha_n, times4,
                         trajectory);
    }
  }
  ...
  GIVEN("Three near points") {
    ...
    THEN("The trajectory is correctly interpolated") {
      ...
      const PredictedTimes times2 =
          compute_predicted_times(times1.t_finish, angle1, w_n, alpha_n);
      test_interpolation(Eigen::Vector3d{x0 - pf, -15.0, theta0},
                         Eigen::Vector3d::UnitZ(), alpha_n, times2, trajectory);

      const PredictedTimes times3 =
          compute_predicted_times(times2.t_finish, pf, v_n, a_n);
      test_interpolation(positions[1], Eigen::Vector3d::UnitY(), a_n, times3,
                         trajectory);

      const PredictedTimes times4 = compute_predicted_times(
          times3.t_finish, std::abs(angle2 - angle1), w_n, alpha_n);
      test_interpolation(Eigen::Vector3d{x0 - pf, -15 + pf, theta0 + angle1},
                         -Eigen::Vector3d::UnitZ(), alpha_n, times4,
                         trajectory);
    }
  }
...
///   rmf_traffic::avg::Interpolate results against the hand-computed
///   expectations
///
