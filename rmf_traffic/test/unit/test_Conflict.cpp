...
/*
####################################################################################################
NOTE: When merging with schedule_tests, the profiles will need to be redefined
with finalized shapes
####################################################################################################
*/
...
SCENARIO("DetectConflict unit tests") {
  ...
  GIVEN(
      "A 2-point trajectory t1 with unit square box profile (stationary "
      "robot)") {
    const double profile_scale = 1.0;
    const rmf_traffic::Time time = std::chrono::steady_clock::now();
    std::shared_ptr<rmf_traffic::geometry::Box> shape =
        std::make_shared<rmf_traffic::geometry::Box>(profile_scale,
                                                     profile_scale);
    rmf_traffic::Trajectory::ProfilePtr profile =
        rmf_traffic::Trajectory::Profile::make_strict(shape);
    ...
    WHEN("t1 and t2 are identical and overlapping") {
      ...
      THEN("The broad phase function detects a conflict") {
        ...
        THEN("The narrow phase function reports the details of conflict") {
          auto narrow_phase_conflicts =
              rmf_traffic::DetectConflict::narrow_phase(t1, t2);
    ...
    WHEN(
        "t2 is t1 but displaced positionally, with bare overlap at the profile "
        "within error margin") {
      rmf_traffic::Trajectory t2("test_map");
      const double dx = 0.8;
  ...
  GIVEN("A stationary trajectory with Box profile") {
    ...
    std::shared_ptr<rmf_traffic::geometry::Box> shape =
        std::make_shared<rmf_traffic::geometry::Box>(box_x_length,
                                                     box_y_length);
    rmf_traffic::Trajectory::ProfilePtr profile =
        rmf_traffic::Trajectory::Profile::make_strict(shape);

    t1.insert(time, profile, Eigen::Vector3d{0, 0, 0},
              Eigen::Vector3d{0, 0, 0});
    t1.insert(time + 10s, profile, Eigen::Vector3d{0, 0, 0},
              Eigen::Vector3d{0, 0, 0});
    REQUIRE(t1.size() == 2);
    ...
    {
      rmf_traffic::Trajectory t2("test_map");
      t2.insert(time, profile, Eigen::Vector3d{0.0, 1.5, 0},
                Eigen::Vector3d{0, 0, 0});
      t2.insert(time + 10s, profile, Eigen::Vector3d{0, 1.5, 0},
                Eigen::Vector3d{0, 0, 0});
      REQUIRE(t2.size() == 2);
...

    {
      ...
      t3.insert(time, profile, Eigen::Vector3d{0, 1.5, 0},
                Eigen::Vector3d{0, 0, 0});
      t3.insert(time + 10s, profile, Eigen::Vector3d{0, 1.5, M_PI_2},
                Eigen::Vector3d{0, 0, 0});
      REQUIRE(t3.size() == 2);
      ...
      THEN("broad_phase should detect collision") {
        ...
        THEN("narrow_phase should return ConflictData") {
          const auto conflicts =
              rmf_traffic::DetectConflict::narrow_phase(t1, t3);
          ...
          // th(t)~=0.65 => t~=4.42
          // box rotating on top collides when the y coordinate of its
          // bottom-left corner equals half the box height
          const double expected_time = 4.42;
          const double computed_time = rmf_traffic::time::to_seconds(
              conflicts.front().get_time() - time);
          CHECK(computed_time ==
                Approx(expected_time).margin(fcl_error_margin));
        }
      }
  ...
  GIVEN("A straight 2-point trajectory") {
    ...
    trajectory_a.insert(begin_time, make_test_profile(UnitBox),
                        Eigen::Vector3d{-5.0, 0.0, 0.0},
                        Eigen::Vector3d{0.0, 0.0, 0.0});

    trajectory_a.insert(begin_time + 10s, make_test_profile(UnitBox),
                        Eigen::Vector3d{5.0, 0.0, 0.0},
                        Eigen::Vector3d{0.0, 0.0, 0.0});
    ...
    WHEN("Checked against a conflicting Trajectory") {
      ...
      trajectory_b.insert(begin_time, make_test_profile(UnitBox),
                          Eigen::Vector3d{0.0, -5.0, 0.0},
                          Eigen::Vector3d{0.0, 0.0, 0.0});

      trajectory_b.insert(begin_time + 10s, make_test_profile(UnitBox),
                          Eigen::Vector3d{0.0, 5.0, 0.0},
                          Eigen::Vector3d{0.0, 0.0, 0.0});
      ...
      CHECK(
          rmf_traffic::DetectConflict::broad_phase(trajectory_a, trajectory_b));

      const auto conflicts =
          rmf_traffic::DetectConflict::narrow_phase(trajectory_a, trajectory_b);
      REQUIRE(conflicts.size() == 1);
    ...
    WHEN("Checked against a Trajectory that does not conflict") {
      ...
      trajectory_c.insert(begin_time, make_test_profile(UnitBox),
                          Eigen::Vector3d{-5.0, 5.0, 0.0},
                          Eigen::Vector3d{0.0, 0.0, 0.0});

      trajectory_c.insert(begin_time + 10s, make_test_profile(UnitBox),
                          Eigen::Vector3d{5.0, 5.0, 0.0},
                          Eigen::Vector3d{0.0, 0.0, 0.0});
      ...
      CHECK(
          rmf_traffic::DetectConflict::broad_phase(trajectory_a, trajectory_c));

      const auto conflicts =
          rmf_traffic::DetectConflict::narrow_phase(trajectory_a, trajectory_c);
  ...
  GIVEN("A curving 2-point trajectory") {
    ...
    rmf_traffic::Trajectory trajectory_a("test_map");
    trajectory_a.insert(begin_time, make_test_profile(UnitCircle),
                        Eigen::Vector3d{-5.0, 0.0, 0.0},
                        Eigen::Vector3d{1.0, 0.0, 0.0});

    trajectory_a.insert(begin_time + 10s, make_test_profile(UnitCircle),
                        Eigen::Vector3d{0.0, -5.0, 0.0},
    ...
    WHEN("Checked against a conflicting Trajectory") {
      rmf_traffic::Trajectory trajectory_b("test_map");
      trajectory_b.insert(begin_time, make_test_profile(UnitCircle),
                          Eigen::Vector3d{-5.0, -5.0, 0.0},
                          Eigen::Vector3d{1.0, 0.0, 0.0});

      trajectory_b.insert(begin_time + 10s, make_test_profile(UnitCircle),
                          Eigen::Vector3d{0.0, 0.0, 0.0},
                          Eigen::Vector3d{0.0, 1.0, 0.0});
      REQUIRE(trajectory_b.size() == 2);
    ...
    WHEN("Checked against a Trajectory that does not conflict") {
      rmf_traffic::Trajectory trajectory_c("test_map");
      trajectory_c.insert(begin_time, make_test_profile(UnitCircle),
                          Eigen::Vector3d{5.0, 0.0, 0.0},
                          Eigen::Vector3d{-1.0, 0.0, 0.0});

      trajectory_c.insert(begin_time + 10s, make_test_profile(UnitCircle),
                          Eigen::Vector3d{0.0, 5.0, 0.0},
                          Eigen::Vector3d{0.0, 1.0, 0.0});
      REQUIRE(trajectory_c.size() == 2);
  ...
  GIVEN("A multi-segment trajectory with straight segments") {
    ...
    t1.insert(time, profile, Eigen::Vector3d{0, 5, 0},
              Eigen::Vector3d{0, 0, 0});
    t1.insert(time + 10s, profile, Eigen::Vector3d{0, -5, 0},
              Eigen::Vector3d{0, 0, 0});
    t1.insert(time + 20s, profile, Eigen::Vector3d{0, -5, M_PI_2},
              Eigen::Vector3d{0, 0, 0});
    t1.insert(time + 30s, profile, Eigen::Vector3d{10, -5, M_PI_2},
              Eigen::Vector3d{0, 0, 0});
    REQUIRE(t1.size() == 4);

    WHEN("Checked with a trajectory that intersects first segment of t1") {
      rmf_traffic::Trajectory t2("test_map");
      t2.insert(time, profile, Eigen::Vector3d{-5, 0, 0},
                Eigen::Vector3d{0, 0, 0});
      t2.insert(time + 10s, profile, Eigen::Vector3d{5, 0, 0},
                Eigen::Vector3d{0, 0, 0});
      REQUIRE(t2.size() == 2);
      ...
      CHECK(conflicts.front().get_segments().first ==
            ++t1.begin());  // segment with the conflict

      const double expected_time = 4.32931077;
      const double computed_time =
          rmf_traffic::time::to_seconds(conflicts.front().get_time() - time);
    ...
    WHEN("Checked with a trajectory that intersects last segment of t1") {
      rmf_traffic::Trajectory t2("test_map");
      t2.insert(time + 20s, profile, Eigen::Vector3d{5, 0, 0},
                Eigen::Vector3d{0, 0, 0});
      t2.insert(time + 30s, profile, Eigen::Vector3d{5, -10, 0},
                Eigen::Vector3d{0, 0, 0});
      REQUIRE(t2.size() == 2);
      ...
      CHECK(conflicts.size() == 1);
      CHECK(conflicts.front().get_segments().first ==
            --t1.end());  // segment with the conflict
    ...
    WHEN(
        "Checked with a multi-segment trajectory that intersects t1 at "
        "two-points") {
      rmf_traffic::Trajectory t2("test_map");
      t2.insert(time, profile, Eigen::Vector3d{-5, -0, -M_PI_4},
                Eigen::Vector3d{0, 0, 0});
      t2.insert(time + 2s, profile, Eigen::Vector3d{0, 0, -M_PI_4},
                Eigen::Vector3d{0, 0, 0});
      t2.insert(time + 10s, profile, Eigen::Vector3d{0, 0, -M_PI_4},
                Eigen::Vector3d{0, 0, 0});
      t2.insert(time + 20s, profile, Eigen::Vector3d{5, -5, M_PI_4},
                Eigen::Vector3d{0, 0, 0});
      t2.insert(time + 30s, profile, Eigen::Vector3d{5, -5, M_PI_4},
                Eigen::Vector3d{0, 0, 0});
      ...
      CHECK(conflicts.front().get_segments().first ==
            ++t1.begin());  // segment with the conflict
      CHECK(conflicts.back().get_segments().first ==
            --t1.end());  // segment with the conflict
    }

    WHEN("Checked with a trajectory that overlaps partially in time") {
      rmf_traffic::Trajectory t2("test_map");
      t2.insert(time + 25s, profile, Eigen::Vector3d{-5, 5, 0},
                Eigen::Vector3d{0, 0, 0});
      t2.insert(time + 35s, profile, Eigen::Vector3d{5, 5, 0},
                Eigen::Vector3d{0, 0, 0});
      REQUIRE(t2.size() == 2);
...
// A useful website for playing with 2D cubic splines:
