...
SCENARIO("Profile unit tests") {
  ...
  GIVEN("Construction values for Profile") {
    std::shared_ptr<rmf_traffic::geometry::Box> unitBox_shape =
        std::make_shared<rmf_traffic::geometry::Box>(1.0, 1.0);
    std::shared_ptr<rmf_traffic::geometry::Circle> unitCircle_shape =
    ...
    WHEN("Constructing a Profile given shape and agency") {
      rmf_traffic::Trajectory::ProfilePtr strict_profile =
          rmf_traffic::Trajectory::Profile::make_strict(unitBox_shape);
      rmf_traffic::Trajectory::ProfilePtr queue_profile =
          rmf_traffic::Trajectory::Profile::make_queued(unitCircle_shape,
                                                        queue_number);

      THEN("Profile is constructed according to specifications.") {
        CHECK(strict_profile->get_shape() == unitBox_shape);
        CHECK(strict_profile->get_agency() ==
        ...
        CHECK(queue_profile->get_agency() ==
              rmf_traffic::Trajectory::Profile::Agency::Queued);
    ...
    WHEN("Pointer for shape used for profile construction is changed") {
      const rmf_traffic::Trajectory::ProfilePtr strict_profile =
          rmf_traffic::Trajectory::Profile::make_strict(unitBox_shape);
      const rmf_traffic::geometry::Box *ptr_address = unitBox_shape.get();
    ...
    WHEN("Shape object used for profile construction is moved") {
      ...
      const rmf_traffic::Trajectory::ProfilePtr strict_profile =
          rmf_traffic::Trajectory::Profile::make_strict(unitBox_shape);
      const std::shared_ptr<rmf_traffic::geometry::Box> new_unitBox_shape =
          std::move(unitBox_shape);
    ...
    WHEN("Queue number used for profile construction is changed") {
      std::string queue_number = "5";
      const rmf_traffic::Trajectory::ProfilePtr strict_profile =
          rmf_traffic::Trajectory::Profile::make_queued(unitBox_shape,
                                                        queue_number);
      queue_number = "6";
  ...
  GIVEN("Sample Profiles and Shapes") {
    const rmf_traffic::Trajectory::ProfilePtr strict_unitbox_profile =
        create_test_profile(UnitBox,
                            rmf_traffic::Trajectory::Profile::Agency::Strict);
    ...
    const std::shared_ptr<rmf_traffic::geometry::Box> new_Box_shape =
        std::make_shared<rmf_traffic::geometry::Box>(2.0, 2.0);

    WHEN("Profile agency is changed using API set_to_* function") {
      THEN("Profile agency is successfully changed") {
        CHECK(strict_unitbox_profile->get_agency() ==
              rmf_traffic::Trajectory::Profile::Agency::Strict);
...
SCENARIO("Segment Unit Tests") {
  ...
  GIVEN("Construction values for Segments") {
    ...
    WHEN(
        "Attemping to construct Segment using "
        "rmf_traffic::Trajectory::insert()") {
      ...
      *strict_unitbox_profile = *queued_unitCircle_profile;
      ...
      THEN("Segment profile is updated.") {
        ...
        const auto &circle = static_cast<const rmf_traffic::geometry::Circle &>(
            *strict_unitbox_profile->get_shape());
    ...
    WHEN("Pointer for profile used for construction is changed") {
      ...
      const rmf_traffic::Trajectory::ProfilePtr new_profile =
          std::move(strict_unitbox_profile);
    ...
    WHEN("Profile used for construction is moved") {
      ...
      rmf_traffic::Trajectory::ProfilePtr new_profile =
          std::move(strict_unitbox_profile);
  ...
  GIVEN("Sample Segment") {
    ...
    inputs.push_back(
        {time, UnitBox, Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0, 0, 0)});
    inputs.push_back({time + 10s, UnitBox, Eigen::Vector3d(1, 1, 1),
                      Eigen::Vector3d(1, 1, 1)});
    inputs.push_back({time + 20s, UnitBox, Eigen::Vector3d(2, 2, 2),
    ...
    WHEN("Setting a new profile using set_profile function") {
      const rmf_traffic::Trajectory::ProfilePtr new_profile =
          create_test_profile(
              UnitCircle, rmf_traffic::Trajectory::Profile::Agency::Autonomous);
    ...
    WHEN(
        "Setting a new finish time that causes a rearrangement of adjacent "
        "segments") {
      ...
      THEN("The appropriate segments are rearranged") {
        ...
        for (rmf_traffic::Trajectory::iterator it = trajectory.begin();
          ...
          CHECK(it->get_finish_position() ==
                Eigen::Vector3d(new_order[i], new_order[i], new_order[i]));
      }
    }

    WHEN(
        "Setting a new finish time that causes a rearrangement of non-adjacent "
        "segments") {
    ...
    WHEN(
        "Positively adjusting all finish times using adjust_finish_times "
        "function, using first segment") {
      const std::chrono::seconds delta_t = std::chrono::seconds(5);
      segment.adjust_finish_times(delta_t);
      int i = 0;
      const rmf_traffic::Time new_order[3] = {time + 5s, time + 15s,
                                              time + 25s};
    ...
    WHEN(
        "Negatively adjusting all finish times using adjust_finish_times "
        "function, using first segment") {
      const std::chrono::seconds delta_t = std::chrono::seconds(-5);
      segment.adjust_finish_times(delta_t);
      int i = 0;
    ...
    WHEN(
        "Large negative adjustment all finish times using adjust_finish_times "
        "function, using first segment") {
      const std::chrono::seconds delta_t = std::chrono::seconds(-50);
      segment.adjust_finish_times(delta_t);
      int i = 0;
      const rmf_traffic::Time new_order[3] = {time - 50s, time - 40s,
                                              time - 30s};

      THEN(
          "All finish times are adjusted correctly, as there is no segment "
    ...
    WHEN(
      ...
      THEN(
          "std::invalid_argument exception thrown due to violation of previous "
          "segment time boundary") {
...
SCENARIO("Trajectory and base_iterator unit tests") {
  ...
  GIVEN("Parameters for insert function") {
    ...
    WHEN("Construct a length 2 trajectory") {
      ...
      auto result = trajectory.insert(
          ...
          pos_0, vel_0);
      const rmf_traffic::Trajectory::iterator zeroth_it = result.it;
      auto result_1 = trajectory.insert(
          time + 10s,
          create_test_profile(UnitBox,
                              rmf_traffic::Trajectory::Profile::Agency::Strict),
          pos_1, vel_1);
    ...
    WHEN("Copy Construction from rvalue base_iterator") {
      ...
      THEN("New iterator is created") {
        const rmf_traffic::Trajectory::iterator &&rvalue_it =
            std::move(zeroth_it);
    ...
    WHEN("Move Construction from another base_iterator") {
      ...
      THEN("New iterator is created") {
        const rmf_traffic::Trajectory::iterator copied_zeroth_it(zeroth_it);
        const rmf_traffic::Trajectory::iterator moved_zeroth_it(
            std::move(copied_zeroth_it));
    ...
    WHEN("Copy Construction of Trajectory from another trajectory") {
      const rmf_traffic::Trajectory trajectory =
          create_test_trajectory(param_inputs);
    ...
    WHEN(
        "Copy Construction of Trajectory followed by move of source "
        "trajectory") {
      const rmf_traffic::Trajectory trajectory =
          create_test_trajectory(param_inputs);
    ...
    WHEN("Appending segment to trajectory") {
      ...
      rmf_traffic::Trajectory::iterator fourth_it =
          trajectory
              .insert(time_3,
                      create_test_profile(
                          UnitBox,
  ...
  GIVEN("Sample Trajectories") {
    ...
    param_inputs.push_back({time + 10s, UnitBox, Eigen::Vector3d(2, 2, 2),
                            Eigen::Vector3d(3, 3, 3)});
    param_inputs.push_back({time + 20s, UnitBox, Eigen::Vector3d(4, 4, 4),
    ...
    WHEN("Finding a segment at the precise time specified") {
      THEN("Segment is retreieved successfully") {
        CHECK(trajectory.find(time)->get_finish_position() ==
              Eigen::Vector3d(0, 0, 0));
        CHECK(trajectory.find(time + 10s)->get_finish_position() ==
              Eigen::Vector3d(2, 2, 2));
        CHECK(trajectory.find(time + 20s)->get_finish_position() ==
    ...
    WHEN("Finding a segment at an offset time") {
      THEN("Segments currently active are retrieved successfully") {
        CHECK(trajectory.find(time)->get_finish_position() ==
              Eigen::Vector3d(0, 0, 0));
        CHECK(trajectory.find(time + 2s)->get_finish_position() ==
              Eigen::Vector3d(2, 2, 2));
        CHECK(trajectory.find(time + 8s)->get_finish_position() ==
              Eigen::Vector3d(2, 2, 2));
        CHECK(trajectory.find(time + 12s)->get_finish_position() ==
              Eigen::Vector3d(4, 4, 4));
        CHECK(trajectory.find(time + 20s)->get_finish_position() ==
              Eigen::Vector3d(4, 4, 4));
      }
    ...
    WHEN("Erasing a first segment") {
      THEN("Segment is erased and trajectory is rearranged") {
        ...
        const rmf_traffic::Trajectory::iterator erase_target =
            trajectory.begin();
        rmf_traffic::Trajectory::iterator next_it =
            trajectory.erase(erase_target);
    ...
    WHEN("Erasing a first segment from a copy") {
      THEN("Segment is erased and only copy is updated, source is unaffected") {
        ...
        const rmf_traffic::Trajectory::iterator next_it =
            trajectory_copy.erase(erase_target);
        CHECK(next_it->get_finish_time() == time + 10s);
    ...
    WHEN("Erasing a empty range of segments using range notation") {
      THEN("Nothing is erased and current iterator is returned") {
        ...
        const rmf_traffic::Trajectory::iterator next_it =
            trajectory.erase(erase_first, erase_last);
        CHECK(trajectory.size() == 3);
    ...
    WHEN("Erasing a empty range of segments from a copy using range notation") {
      THEN("Nothing is erased") {
        ...
        const rmf_traffic::Trajectory::iterator erase_last = erase_first;
        const rmf_traffic::Trajectory::iterator next_it =
            trajectory_copy.erase(erase_first, erase_last);
        CHECK(trajectory_copy.size() == 3);
        CHECK(trajectory.size() == 3);
        CHECK(next_it->get_finish_time() == time);
      }
    ...
    WHEN(
        "Erasing the first and second segments of a copy using range "
        "notation") {
      THEN("2 Segments are erased and trajectory is rearranged") {
        rmf_traffic::Trajectory trajectory_copy = trajectory;
        CHECK(trajectory_copy.size() == 3);
