...
inline void CHECK_broad_phase_is_commutative(rmf_traffic::Trajectory t1,
                                             rmf_traffic::Trajectory t2) {
  CHECK(rmf_traffic::DetectConflict::broad_phase(t1, t2) ==
        rmf_traffic::DetectConflict::broad_phase(t2, t1));
}

inline void CHECK_narrow_phase_is_commutative(rmf_traffic::Trajectory t1,
                                              rmf_traffic::Trajectory t2) {
  ...
  for (auto i_1 = conflicts_1.begin(), i_2 = conflicts_2.begin();
    ...
    rmf_traffic::Trajectory::const_iterator conflict_1_t1 =
        i_1->get_segments().first;
    rmf_traffic::Trajectory::const_iterator conflict_2_t1 =
        i_2->get_segments().second;
    CHECK(conflict_1_t1 == conflict_2_t1);

    rmf_traffic::Trajectory::const_iterator conflict_1_t2 =
        i_1->get_segments().second;
    rmf_traffic::Trajectory::const_iterator conflict_2_t2 =
        i_2->get_segments().first;
    CHECK(conflict_1_t2 == conflict_2_t2);
  }
}

inline void CHECK_between_is_commutative(rmf_traffic::Trajectory t1,
                                         rmf_traffic::Trajectory t2) {
...
inline void CHECK_ConflictData(
    rmf_traffic::ConflictData data, const rmf_traffic::Time start_time,
  ...
  // t1 and t2 are precisely pointing to the Trajectory segments we are trying
  // to verify, Not a "mock up" segment created separately Note that the
  // const_iterators should not be the first in a trajectory: ie
  // trajectory.begin() ConflictData will not return the first iterator as it
  // only represents a point in time and cannot "conflict"

  rmf_traffic::Trajectory::const_iterator t1_conflict_segment =
      data.get_segments().first;
  rmf_traffic::Trajectory::const_iterator t2_conflict_segment =
      data.get_segments().second;
  CHECK(t1_conflict_segment == t1_expected_conflict_segment);
  CHECK(t2_conflict_segment == t2_expected_conflict_segment);

  // We can only test for approximate accuracies due to the numerical nature of
  // FCL
  const double expected_duration =
      rmf_traffic::time::to_seconds(expected_conflict_time - start_time);
  const double computed_duration =
      rmf_traffic::time::to_seconds(data.get_time() - start_time);
...
inline void CHECK_ConflictList(
    std::vector<rmf_traffic::ConflictData> computed_conflicts,
    std::vector<ConflictDataParams> expected_conflicts) {
  std::vector<rmf_traffic::ConflictData>::iterator c_it =
      computed_conflicts.begin();
  ...
  for (; c_it != computed_conflicts.end(); ++c_it, ++exp_it) {
    ...
    CHECK_ConflictData(data, start_time, expected_conflict_time,
