...
//==============================================================================
enum TestProfileType { UnitBox, UnitCircle };
...
inline rmf_traffic::Trajectory::ProfilePtr create_test_profile(
  ...
  if (UnitBox == shape &&
      rmf_traffic::Trajectory::Profile::Agency::Strict == agency) {
  ...
  } else if (UnitBox == shape &&
             rmf_traffic::Trajectory::Profile::Agency::Queued == agency) {
...
inline rmf_traffic::Trajectory create_test_trajectory(
  ...
  for (auto x : param_list) {
    trajectory.insert(x.time, create_test_profile(x.profile_type), x.pos,
                      x.vel);
  }
  return trajectory;
}
...
inline rmf_traffic::Trajectory make_test_trajectory(rmf_traffic::Time t,
                                                    int length, int dur) {
